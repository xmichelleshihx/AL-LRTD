...
class NonLocalBlock_test(nn.Module):
    def __init__(self, dim_in, dim_out, dim_inner):
...
class NonLocalBlock_test_Conv1(nn.Module):
    def __init__(self, dim_in, dim_out, dim_inner):
        # B x C x T x 1 x 1
...
#-----------------------------------------------------------------------------------------------#
'''
class I3Res50_new_test(nn.Module):

    def __init__(self, block=Bottleneck_test, layers=[3, 4, 6, 3], num_classes=400, use_nl=False):
        self.inplanes = 64
    ...
    def forward_main_layer1(self, x):
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.relu(x)
        # x = self.maxpool1(x)
...
class I3Res50_new_test_1block(nn.Module):
...
def i3_res50_nl(num_classes,pretrained=False):
...
def i3_res50_nl_new(num_classes,pretrained=False):
...
def i3_res50_nl_new_test(num_classes,pretrained=False):
...
def i3_res50_nl_new_test_1block(num_classes,pretrained=False):
